 ...
 *
 *
 * Name:        AGBA, THOMAS AGBA
 * Reg. No:   	PDS/2013/5860
...
#include "HealthProfile.h"  // include definition of HealthProfile class
using namespace std;

// constructor initializes patient information
HealthProfile::HealthProfile(string firstName, string lastName, string gender, int month, int day, int year, int weight, double height,
							int thisDay, int thisMonth, int thisYear)
{
    ...
    setWeight(weight);
    setHeight(height);
    setAge(thisDay, thisMonth, thisYear);
}
...
{
	int x = 0;
	if ((getMonth() == thisMonth) && (getDay() > thisDay)) x  = -1;
	if (getMonth() > thisMonth) x = -1;
	age = (thisYear - getYear() + x);	
}
